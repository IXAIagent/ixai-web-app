  ...
  return (
    <section className="rounded-2xl border border-[rgba(9,41,31,0.14)] bg-white/82 p-5 shadow-[0_18px_48px_rgba(9,41,31,0.06)] sm:p-6">
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <h3 className="text-sm font-semibold text-[var(--ixai-forest)]">
                {t(`v2Insight_${insight.id}_title`, insight.title)}
              </h3>
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--ixai-forest-soft)]">
              {t(`v2Insight_${insight.id}_summary`, insight.summary)}
            </p>
          </article>
        ))}
        {!report ? (
      ...
      <p className="mt-4 rounded-xl border border-[var(--ixai-border)] bg-white/60 p-3 text-xs leading-6 text-[var(--ixai-forest-soft)]">
        {t("safetyFlags")}: {t("aiProvider")} {String(report?.safetyFlags.aiProviderEnabled ?? false)},
        {t("recommendations")} {String(report?.safetyFlags.recommendationLogicEnabled ?? false)},
        {t("actionableTradingInstructions")} {String(report?.safetyFlags.actionableTradingInstructions ?? false)}.
      </p>
    </section>
  );
}
