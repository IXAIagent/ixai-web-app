...
- V12.1 Runtime Stabilization Program is partial complete, not complete. Program A — Root Provider Stabilization is complete via PR #75 and commit `9c73915`, hardening root provider async boundaries in `components/auth/auth-provider.tsx` and `components/auth/identity-provider.tsx` so Supabase auth/session, storage, and network failures degrade safely instead of surfacing as global unhandled promise storms. Program B — Workspace Runtime Stabilization, Program C — Market / Morning Brief Runtime Stabilization, and Program D — Admin / Scheduler Runtime Stabilization remain pending. Program E service worker safety also remains deferred.
