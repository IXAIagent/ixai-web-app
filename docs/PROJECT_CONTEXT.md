...
V13.5 Full Translation Coverage Program follows the blocked Batch 3 visual QA note and repairs high-exposure source-level translation coverage. It expands dictionary coverage, adds non-English pack mappings for `zh-CN`, `ja-JP`, and `ko-KR`, and wires Settings diagnostics, Risk Center, and Intelligence Center display labels to the shared i18n layer. This batch does not change auth, RLS, schema, migrations, API contracts, billing, broker integration, trading, recommendations, scheduler/notification delivery, OpenAI, AI model behavior, live market provider behavior, exchange-rate provider behavior, valuation engine, risk engine, or FCN engine logic. See `docs/V135_FULL_TRANSLATION_COVERAGE.md`, `docs/V123_TRANSLATION_MAINTENANCE_BATCH3.md`, `docs/TRANSLATION_VISUAL_QA_BATCH_2.md`, `docs/TRANSLATION_MAINTENANCE_BATCH_1.md`, `docs/V13_SPRINT_4_AUTHENTICATED_WORKSPACE_TRANSLATION_AUDIT.md`, and `docs/V13_SPRINT_3_TRANSLATION_COVERAGE_REPAIR.md`.

V13 infrastructure foundation is complete after Sprint 2, but product translation coverage must be judged by production visual QA. Sprint 3 was the first coverage repair pass; Sprint 4 repaired Portfolio authenticated readback first; Batch 1 started the authenticated Workspace maintenance track; Batch 2 confirmed that a signed-in production browser session is required before authenticated Workspace visual QA can be claimed; Batch 3 remained blocked by the same unauthenticated browser state. V13.5 repairs the highest-exposure dictionary and UI label gaps in Settings, Risk, Intelligence, and non-English locale mapping. Deep FCN, Input, editorial/public long-form content, and admin/legacy preview surfaces remain candidates for targeted dictionary migration after signed-in manual verification.
...
- Recommended next options are signing into production Workspace and rerunning Batch 3, V14 Beta Production Verification / Invite-only Pilot, V15 AI Wealth OS planning, or deeper dictionary migration as maintenance for remaining long-tail surfaces.
