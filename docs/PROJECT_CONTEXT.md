...
V12 Translation Maintenance Program Batch 3 follows Batch 2 and attempts authenticated production Workspace visual QA on `https://app.ixuan.ai`. The available browser session is not signed in, so protected Workspace visual QA is blocked and not claimed complete. Batch 3 creates a route inventory and records the authentication prerequisite for the next signed-in run. This batch does not change auth, RLS, schema, migrations, API contracts, billing, broker integration, trading, recommendations, scheduler/notification delivery, OpenAI, AI model behavior, live market provider behavior, exchange-rate provider behavior, valuation engine, risk engine, or FCN engine logic. See `docs/V123_TRANSLATION_MAINTENANCE_BATCH3.md`, `docs/TRANSLATION_VISUAL_QA_BATCH_2.md`, `docs/TRANSLATION_MAINTENANCE_BATCH_1.md`, `docs/V13_SPRINT_4_AUTHENTICATED_WORKSPACE_TRANSLATION_AUDIT.md`, and `docs/V13_SPRINT_3_TRANSLATION_COVERAGE_REPAIR.md`.

V13 infrastructure foundation is complete after Sprint 2, but product translation coverage must be judged by production visual QA. Sprint 3 was the first coverage repair pass; Sprint 4 repaired Portfolio authenticated readback first; Batch 1 started the authenticated Workspace maintenance track; Batch 2 confirmed that a signed-in production browser session is required before authenticated Workspace visual QA can be claimed; Batch 3 remains blocked by the same unauthenticated browser state. Deep Risk, FCN, Intelligence, FCN input forms, and long-form public content remain candidates for targeted dictionary migration after signed-in manual verification.
