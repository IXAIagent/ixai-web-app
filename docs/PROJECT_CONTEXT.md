...
Translation Maintenance Batch 2 follows Batch 1 after production review confirmed that source-level translation scans were not enough. Batch 2 makes V13 translation maintenance Visual QA driven, attempts production Workspace verification on `https://app.ixuan.ai`, documents that the available browser session was not authenticated, scans public routes in `zh-TW`, `en-US`, and `ja-JP`, fixes first-pass high-impact FCN display labels, and records that `ja-JP` is metadata-supported but not full content coverage. This batch does not change auth, RLS, schema, migrations, API contracts, billing, broker integration, trading, recommendations, scheduler/notification delivery, OpenAI, AI model behavior, live market provider behavior, exchange-rate provider behavior, valuation engine, risk engine, or FCN engine logic. See `docs/TRANSLATION_VISUAL_QA_BATCH_2.md`, `docs/TRANSLATION_MAINTENANCE_BATCH_1.md`, `docs/V13_SPRINT_4_AUTHENTICATED_WORKSPACE_TRANSLATION_AUDIT.md`, and `docs/V13_SPRINT_3_TRANSLATION_COVERAGE_REPAIR.md`.

V13 infrastructure foundation is complete after Sprint 2, but product translation coverage must be judged by production visual QA. Sprint 3 was the first coverage repair pass; Sprint 4 repaired Portfolio authenticated readback first; Batch 1 started the authenticated Workspace maintenance track; Batch 2 confirms that a signed-in production browser session is required before authenticated Workspace visual QA can be claimed. Deep Risk, FCN, Intelligence, FCN input forms, and long-form public content remain candidates for targeted dictionary migration after manual verification.
...
- Recommended next options are Translation Maintenance Batch 3 with authenticated production Workspace access, V14 Beta Production Verification / Invite-only Pilot, V15 AI Wealth OS planning, or deeper dictionary migration as maintenance for remaining long-tail surfaces.
